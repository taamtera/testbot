...
class Task:
    """
    TASK: Help Turtle to visit 40 of his friends.
    Their houses are always at fixed locations.
    """
    def start(self):
        stage.reset()
        self.friends = []
        for i in range(0,360,45):
            for j in range(1,6):
                x = j*50*math.cos(i*math.pi/180)
                y = j*50*math.sin(i*math.pi/180)
                friend = Home(x,y,32,32,IMG_HOME)
                self.friends.append(friend)
                stage.add_object(friend)
        if INTERACTIVE:
    ...
    def done(self):
        ...
        def visited(friend):
            return any([friend.contains(stop) for stop in stage.stops])
        ...
        visited_friends = sum(visited(friend) for friend in self.friends)

        result = "PASSED" if num_friends == visited_friends else "FAILED"
        print(f"Turtle visited {visited_friends} out of {num_friends}: {result}")
